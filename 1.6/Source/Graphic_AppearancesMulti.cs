...
{
    public class Graphic_AppearancesMulti : Graphic
    {
        ...
        public override Material MatSingle
        {
            get
            {
                return subGraphics[StuffAppearanceDefOf.Smooth.index].MatSingle;
            }
        }
        ...
        public override void Init(GraphicRequest req)
        {
            ...
            for (int i = 0; i < subGraphics.Length; i++)
            {
                ...
                Texture2D texture2D = (from x in ContentFinder<Texture2D>.GetAllInFolder(text)
                                       where ReplaceName(x.name).EndsWith(stuffAppearance.defName)
                                       select x).FirstOrDefault();
                if (texture2D != null)
                {
                    var path = text + "/" + ReplaceName(texture2D.name);
                    subGraphics[i] = GraphicDatabase.Get<Graphic_Multi>(path, req.shader, drawSize, color);
                }
            }
        ...
        {
            return name.Replace("_north", "").Replace("_east", "").Replace("_south", "");
        }
        ...
        public override Graphic GetColoredVersion(Shader newShader, Color newColor, Color newColorTwo)
        {
            ...
            return GraphicDatabase.Get<Graphic_AppearancesMulti>(path, newShader, drawSize, newColor, Color.white, data);
        }

        public override Material MatSingleFor(Thing thing)
        {
            return SubGraphicFor(thing).MatAt(thing.Rotation, thing);
        }
